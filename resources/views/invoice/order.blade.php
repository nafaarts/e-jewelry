...
<head>
    ...
    <title>{{ 'Tempahan | #' . $order->order_number }}</title>
    ...
    <style>
        * {
        ...
        @page {
            size: "{{ $config['invoice_order_paper_size'] }}";
            margin: 0;
        }
...
<body>
    <div class="page" id="printable-area">
        <img style="width: 100%; height: auto;"
            src="{{ convertImageToBase64('storage/' . $config['invoice_order_header_image']) }}" alt="header">

        <div style="{{ $config['invoice_order_paper_size'] == 'A4' ? 'padding: 20px 40px;' : 'padding: 10px 20px;' }}">
            <h1 class="text-md font-bold my-3 text-center underline">TEMPAHAN</h1>
                    ...
                    <td class="w-half align-top">
                        ...
                        <table class="text-xs">
                            ...
                            <tr>
                                ...
                                <td class="px-2">:</td>
                                <td>{{ $order->estimated_date? now()->parse($order->estimated_date)->translatedFormat('d F Y'): '-' }}
                                </td>
                            </tr>
                        </table>
                    </td>
                ...
                <tr>
                    <td class="w-half align-top">
                        <table>
                            ...
                            <tr>
                                <td class="text-xs text-grey">Kadar</td>
                                <td class="text-xs px-2">:</td>
                                <td class="text-xs">{{ $order->saved_price->carat }}
                                    ({{ $order->saved_price->rate }}%)</td>
                            </tr>
                            <tr>
                                <td class="text-xs text-grey">Harga</td>
                                <td class="text-xs px-2">:</td>
                                <td class="text-xs">
                                    Rp. {{ number_format($order->saved_price->sell_price, 0, ',', '.') }}
                                    <span class="text-grey lowercase">/{{ $order->saved_price->category }}</span>
                                </td>
                            </tr>
                        </table>
                    </td>
                ...
                <tr>
                    <td style="width: 70%;">
                        <div style="margin-bottom: 1rem">
                            ...
                            <p class="text-10 text-grey white-space-preline">{{ $config['invoice_order_note'] }}
                            </p>
                        </div>
