...
<head>
    ...
    <title>{{ 'Titipan | #' . $transaction->transaction_number }}</title>
    ...
    <style>
        * {
        ...
        @page {
            size: "{{ $config['invoice_deposit_paper_size'] }}";
            margin: 0;
        }
...
<body>
    <div class="page" id="printable-area">
        <img style="width: 100%; height: auto;"
            src="{{ convertImageToBase64('storage/' . $config['invoice_deposit_header_image']) }}" alt="header">

        <div
            style="{{ $config['invoice_deposit_paper_size'] == 'A4' ? 'padding: 20px 40px;' : 'padding: 10px 20px;' }}">
            <h1 class="text-md font-bold my-3 text-center underline">TITIPAN</h1>
            ...
            <table class="w-full">
                <tr>
                    <td class="w-half align-top">
                        ...
                        <table class="text-xs">
                            <tr>
                                <td class="text-grey">No. Transaksi</td>
                                <td class="px-2">:</td>
                                <td>{{ $transaction->transaction_number }}</td>
                            </tr>
                            <tr>
                                <td class="text-grey">Rekening</td>
                                <td class="px-2">:</td>
                            ...
                            <tr>
                                ...
                                <td class="px-2">:</td>
                                <td>{{ $transaction->updated_at->translatedFormat('d F Y') }}</td>
                            </tr>
                            <tr>
                                <td class="text-xs text-grey">Jenis</td>
                                <td class="text-xs px-2">:</td>
                                <td class="text-xs font-bold">{{ $transaction->type == 'CREDIT' ? 'TITIP' : 'AMBIL' }}
                                </td>
                            </tr>
                            <tr>
                                <td class="text-xs text-grey">Kategori</td>
                                <td class="text-xs px-2">:</td>
                                <td class="text-xs">{{ $transaction->category == 'GOLD' ? 'EMAS' : 'UANG' }}</td>
                            </tr>

                            @if ($transaction->category == 'GOLD')
                                <tr>
                            ...
                            @else
                                <tr>
                                    <td class="text-xs text-grey">Jumlah</td>
                                    <td class="text-xs px-2">:</td>
                                    <td class="text-xs font-bold">
                            ...
                            <tr>
                                <td class="text-xs text-grey">Biaya</td>
                                <td class="text-xs px-2">:</td>
                                <td class="text-xs">
                                    Rp. {{ number_format($transaction->cost) }}
                                </td>
                            </tr>
                    ...
                    <td class="w-half align-top">
                        ...
                        <table class="text-xs">
                            ...
                            <tr>
                                ...
                                <td class="px-2">:</td>
                                <td>{{ $account->costumer->phone_number }}</td>
                            </tr>
                        </table>
                    </td>
                </tr>
                ...
                <tr>
                    <td style="width: 70%;">
                        <div style="margin-bottom: 1rem">
                            ...
                            <p class="text-10 text-grey white-space-preline">{{ $config['invoice_deposit_note'] }}
                            </p>
                        </div>
